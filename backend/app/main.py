...
def _route_template(request: Request) -> str:
    """Full route template, e.g. /api/v1/specs/{request_id}.

    Included routers are not flattened, so the matched route only knows its
    path relative to its router; the prefix is recovered from the concrete URL.
    """
